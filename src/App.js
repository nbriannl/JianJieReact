...
class App extends Component {
  constructor(props) {
    ...
    const listOne = <ToDoList cardColor='green' /> 
    const listTwo = <ToDoList cardColor='blue' /> 
    this.state = {
        todolists: [],
    };
  ...
  handleOnClick() {
    ...
    let color = colors[this.state.todolists.length % colors.length];
    const addedList = <ToDoList cardColor={color} />     
    todolists.push(addedList)
  ...
  render() {
    return (
      ...
      <Button className="Aligner-item" onClick={this.handleOnClick}>Add</Button>
        {this.state.todolists.map(function mapTodoLists(todoList) {
          return <div className="Aligner-item toDoList">{todoList}</div>;
        })}
        {/* todolist => <div className="Aligner-item toDoList">{todolist}</div>)} */}
      </div>
    )
  }
}
